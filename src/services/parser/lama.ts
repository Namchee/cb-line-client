import { Parser, Bidang, MahasiswaInfo, BadNPMError, jenjang, fakultas } from './types';
...
const jurusan: Map<string, Bidang> = new Map([
  ...
  ['891', { ilmu: 'Magister Hubungan Internasional', jenjang: '8', fakultas: '8' }],
]);
...
export class ParserLama implements Parser {
  public parse(npm: string): MahasiswaInfo {
    ...
    if (!this.isValidFormat(npm) || !bidang_studi) {
    ...
    return {
      ...
      bidang_studi: {
        ...
        fakultas: fakultas.get(bidang_studi.fakultas || '') || '',
        jenjang: jenjang.get(bidang_studi.jenjang || '') || '',
      },
